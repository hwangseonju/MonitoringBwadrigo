...
function Application(){
    ...
    const changeHandler = (checked, id) => {
        if(checked){
            setCheckValue([...checkValue, {collectFortype : id}]);
            console.log(id,"체크");
        }
        else{
            setCheckValue(checkValue.filter(e => !e.collectFortype.includes(id)));
    ...
    return(
        <div>
            ...
            <Form>
                ...
                checked = {checkValue.filter((e)=>{return e.collectFortype.includes('wash')}).length>0 ? true : false} 
                onChange={e => {
                    changeHandler(e.currentTarget.checked,e.currentTarget.id);
                    }} 
                />
                <Form.Check type='checkbox' id='bedding' label='이불' 
                checked = {checkValue.filter((e)=>{return e.collectFortype.includes('bedding')}).length>0 ? true : false} 
                onChange={e => {
                    changeHandler(e.currentTarget.checked,e.currentTarget.id);
                    }} 
                /><Form.Check type='checkbox' id='shirts' label='와이셔츠' 
                checked = {checkValue.filter((e)=>{return e.collectFortype.includes('shirts')}).length>0 ? true : false} 
                onChange={e => {
                    changeHandler(e.currentTarget.checked,e.currentTarget.id);
                    }} 
                /><Form.Check type='checkbox' id='cleaning' label='드라이 클리닝' 
                checked = {checkValue.filter((e)=>{return e.collectFortype.includes('cleaning')}).length>0 ? true : false} 
                onChange={e => {
                    changeHandler(e.currentTarget.checked,e.currentTarget.id);
                    }} 
                />
